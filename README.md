...
Agent 稳定性 P1 已完成五类基础问答真机验收。P2-1 已完成：ReAct 默认最多执行 4 轮，规划达到上限后停止继续调用 LLM，服务端 Collector 回归 11/11 通过；后续继续处理重复失败 Tool、阶段预算、客户端断开和统一超时映射。

## 下一步

1. P1 基础问答稳定性验收已完成：历史、天气、景点、预算和路线五类问答均已在真实手机上验证。
2. P2-2：停止重复失败 Tool，并回归同一 Tool 的终态失败行为。
