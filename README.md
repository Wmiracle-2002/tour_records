...
新增旅行的城市选择器现只展示地级目的地、四个直辖市及没有地级上级的省直管县级市，共 358 个选项；区县名称仍可用于搜索上级城市，例如输入“昆山”会显示“苏州市”。已有区县记录不迁移，Agent 的天气/地点查询也不受旅行选择器粒度影响。短期会话与长期偏好记忆的待讨论设计见 [Agent记忆系统方案.md](Agent记忆系统方案.md)。
