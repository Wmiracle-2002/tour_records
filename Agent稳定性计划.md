...
## 7. Agent 最小化改进方案 E 步验收（2026-09-26）

- 服务端以认证 HTTP 请求模拟 Android 消息，覆盖历史记录、地点推荐、距离、天气、人民币预算及粗粒度行程；检查回答、`request_id` 关联、阶段耗时及高德实际请求路径。
- 五种降级场景覆盖无历史、无有效 POI、无目标日期预报、预算缺少天数、规划缺少候选；均返回说明，不补造地点、天气或价格。规划分别收集景点和餐饮，避免只有景点的稀疏行程。
- 真实高德联调确认 `extensions=base` 的 POI 可以没有 `adcode`，因此用精确城市名验证后接收；无城市名或错城仍拒绝。南京景点、餐饮、行政区、天气及“中山陵景区”到夫子庙的直线距离均已查通。
- 本地模拟测试不代表真实 LLM 和手机验收；部署后还需重点观察一日/三日规划耗时、候选质量、天气日期及请求 ID。旧的路线导航验收条目属于先前版本；当前版本只保留距离测量，不提供具体导航。
