...
当前进度：P2-1 至 P2-4 已完成。ReAct Collector 的默认最大轮数从 8 轮收紧为 4 轮，达到上限后不再发起新的决策或 Tool 调用；已经进入 `completed`、`unavailable` 或 `failed` 终态的信息 Tool 不再暴露给下一轮 ReAct；新增整条请求和各 LLM 阶段预算，预算会覆盖当前 HTTP 请求的读取超时并在耗尽后停止后续阶段；API 会监测客户端断开并通过线程安全取消信号阻止后续 Agent 阶段。P2 相关定向回归 65/65 通过。
